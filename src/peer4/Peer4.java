...
public class Peer4 {
	...
	static String client4_location = rootDirectory + "/peer4";
	static String chunksLocation = client4_location + "/chunks";
	...
	public static void main(String[] args) {
		...
		new File(client4_location + "/chunks/").mkdirs();
	...
	public void TCPCliConnect(int port) throws InterruptedException {
		...
		try {
			...
			System.out.println(" client4 connected to :" + clientSocket);
	...
	public void TCPCliDisconnect() {
		try {
			inStream.close();
			clientSocket.close();
			System.out.println("Client4 connection closed: "+clientSocket);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public void combineChunks() {
		...
		String chunksLocation = client4_location + "/chunks";
		File[] files = new File(chunksLocation).listFiles();
		byte[] chunk = new byte[102400];
		try {
			FileOutputStream fileOutStream = new FileOutputStream(
					new File(client4_location +"/" +(files[0].getName()).split("_")[1]));
			BufferedOutputStream bufferOutStream = new BufferedOutputStream(fileOutStream);
			for (File f : files) {
