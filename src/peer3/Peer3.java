...
public class Peer3 {
	...
	static String client3_location = rootDirectory + "/peer3";
	static String chunksLocation = client3_location + "/chunks";
	...
	public static void main(String[] args) {
		...
		new File(client3_location + "/chunks/").mkdirs();
		...
		System.out.println("Client2 totalFilesToReceive: "+totalFilesToReceive);
	...
	public void TCPCliConnect(int port) throws InterruptedException {
		...
		try {
			...
			System.out.println("Client3 connected to :" + clientSocket);
	...
	public void TCPCliDisconnect() {
		try {
			inStream.close();
			clientSocket.close();
			System.out.println("Client3 connection closed: "+clientSocket);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public void combineChunks() {
		...
		String chunksLocation = client3_location + "/chunks";
		File[] files = new File(chunksLocation).listFiles();
		byte[] chunk = new byte[102400];
		try {
			FileOutputStream fileOutStream = new FileOutputStream(
					new File(client3_location +"/" + (files[0].getName()).split("_")[1]));
			BufferedOutputStream bufferOutStream = new BufferedOutputStream(fileOutStream);
			for (File f : files) {
