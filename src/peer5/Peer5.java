...
public class Peer5 {
	...
	static String client5_location = rootDirectory + "/peer5";
	static String chunksLocation = client5_location + "/chunks";
	...
	public static void main(String[] args) {
		...
		new File(client5_location + "/chunks/").mkdirs();
		...
		System.out.println("Client5 totalFilesToReceive:"+totalFilesToReceive);
		while(true)
		{
	...
	public void TCPCliConnect(int port) throws InterruptedException {
		...
		try {
			...
			System.out.println(" client5 connected to :" + clientSocket);
	...
	public void TCPCliDisconnect() {
		try {
			inStream.close();
			clientSocket.close();
			System.out.println("Client5 connection closed: "+clientSocket);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public void combineChunks() {
		...
		String chunksLocation = client5_location + "/chunks";
		File[] files = new File(chunksLocation).listFiles();
		byte[] chunk = new byte[102400];
		try {

			FileOutputStream fileOutStream = new FileOutputStream(
					new File(client5_location +"/" + (files[0].getName()).split("_")[1]));
			BufferedOutputStream bufferOutStream = new BufferedOutputStream(fileOutStream);
			for (File f : files) {
